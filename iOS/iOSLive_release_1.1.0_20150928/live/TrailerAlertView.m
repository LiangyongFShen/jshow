...
- (id)init{
    self = [[[NSBundle mainBundle] loadNibNamed:@"TrailerAlertView" owner:self options:nil] lastObject];
    if(self){
...
- (void)showTime:(NSString*)time logo:(NSString*)logo name:(NSString*)name praise:(NSString*)praise{
    ...
    if([logo isEqualToString:@""]){
        ...
        [self.logoImageView sd_setImageWithURL:[NSURL URLWithString:logoUrl] placeholderImage:[UIImage imageWithColor:RGB16(COLOR_FONT_WHITE) andSize:self.logoImageView.frame.size]];
    }
    self.alpha=0;
