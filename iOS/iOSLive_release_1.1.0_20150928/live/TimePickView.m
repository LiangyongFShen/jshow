...
- (id)init{
    self = [[[NSBundle mainBundle] loadNibNamed:@"TimePickView" owner:self options:nil] lastObject];
    if(self){
...
- (void)setLeftTime{
    ...
    if((dayRow > dayCur) ||
       ...
       (dayRow == dayCur && hourRow == hourCur && minuteRow*10 > minuteCur)){
        ...
        if(0 != hourLeft){
            leftStr = [leftStr stringByAppendingString:[NSString stringWithFormat:@"%d小时",hourLeft]];
        }
        if(0 != minuteLeft){
            leftStr = [leftStr stringByAppendingString:[NSString stringWithFormat:@"%d分钟后",minuteLeft]];
        }
        else{
