...
- (void)showWithName:(NSString*)name signature:(NSString*)sig praise:(NSString*)praise logo:(NSString*)logo{
    ...
    if([logo isEqualToString:@""]){
        ...
        [self.userLogoImageView sd_setImageWithURL:[NSURL URLWithString:logoUrl] placeholderImage:[UIImage imageWithColor:RGB16(COLOR_FONT_WHITE) andSize:self.userLogoImageView.frame.size]];
    }
